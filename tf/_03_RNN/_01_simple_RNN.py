...
步骤：
    1.  
                X（batch中的一个）                          state（既是输出又是输入）                                       W             b
        concat(batchsize x classes ,batchsize x state_size) x (classes + state_size) + b
        state = concat(5x2 , 5x10) x ((2+10)x10) + (10) = 5 x 10 是一个窗口，每次迭代带着上一次的属性
'''
...
def rnn_cell(rnn_input,state):
    with tf.variable_scope('rnn_cell',reuse=True):
        W = tf.get_variable('W', [n_classes + state_size, state_size])
        b = tf.get_variable('b', [state_size], initializer=tf.constant_initializer(0.0))
        rr = tf.tanh(tf.matmul(tf.concat((rnn_input,state),1),W)+b)
...
#                 print(X,'--',Y)
                tr_losses, training_loss_, training_state, _,x_one_hot1,rnn_inputs1,state1,rnn_outputs1 = \
                    sess.run([losses,
                              total_loss,
                              final_state,
                              train_step,x_one_hot,rnn_inputs,state,rnn_outputs],
                             feed_dict={x: X, y: Y, init_state: training_state})
                training_loss += training_loss_
#                 print('x_one_hot1:',x_one_hot1,'rnn_inputs1:',rnn_inputs1)
                print('rnn_outputs1:\n',len(rnn_outputs1))
