...
class TripleExtractor:
    ...
    '''三元组抽取主函数'''
    def ruler2(self, words, postags, child_dict_list, arcs, roles_dict,needRule1=False):
        svos = []
    ...
    '''程序主控函数'''
    def triples_main(self, content,needRule1=False):
        sentences = self.split_sents(content)
        svos = []
        for sentence in sentences:
            words, postags, child_dict_list, roles_dict, arcs = self.parser.parser_main(sentence)
            svo = self.ruler2(words, postags, child_dict_list, arcs, roles_dict,needRule1)
            svos += svo
...
def test():
    ...
    content6='''5月7日20时许，昌平警方针对霍营街道某小区一足疗店存在卖淫嫖娼问题的线索，组织便衣警力前往开展侦查。
...
　　由雷洋的同学发布的一份情况说明称，5月7日，由于雷洋夫妇刚得一女，其亲属欲来京探望，航班预计当晚23点30分到达。当晚21时左右，雷洋从家里出门去首都机场迎接亲属，之后雷洋失联。（来源：央视、新京报）'''
    ...
    while 1:
        ...
        svos = extractor.triples_main(question,True)
        print('有1的结果:', svos)
        svos = extractor.triples_main(question, False)
        print('无1的结果:', svos)
