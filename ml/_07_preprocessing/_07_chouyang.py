...
# 简单随机抽样
data = [5.1 ,4.9, 4.7 ,4.6, 5.,  5.4, 4.6, 5. ] # 导入普通数据文件
data_sample = random.sample(data, 5) # 随机抽取2000个样本
print (data_sample[:2]) # 打印输出前2条数据
print (len(data_sample)) # 打印输出抽样样本量
# 等距抽样
data = np.loadtxt('demo2_Iris.txt') # 导入普通数据文件
...
# 分层抽样
# 导入有标签的数据文件
data2 = np.loadtxt('demo2_Iris.txt') # 导入带有分层逻辑的数据
each_sample_count = 20 # 定义每个分层的抽样数量
label_data_unique = np.unique(data2[:, -1]) # 定义分层值域,该函数是去除数组中的重复数字，并进行排序之后输出。
...
# 整群抽样
data3 = np.loadtxt('demo2_Iris.txt') # 导入已经划分好整群的数据集
...
#将数据分成训练街、测试集
data4 = np.loadtxt('demo2_Iris.txt') # 导入已经划分好整群的数据集
x = data4[:,:-1]
y = data4[:,-1]
