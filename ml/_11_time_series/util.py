...
def household_power2machine():
    filename = 'household_power_consumption.txt'
    df = pd.read_csv(filename, header=0, delimiter=';', sep=',')
    df = df.replace('?', np.NaN)
    df.dropna(inplace=True)
    mean1 = df['Global_active_power'].astype('float').mean()
    print(mean1)
    df.loc[df['Global_active_power'].astype('float') < mean1, 'Global_active_power'] = 0
    df.loc[df['Global_active_power'].astype('float') >= mean1, 'Global_active_power'] = 1
    print(df)
    df.to_csv('household_power_consumption1.txt', index=False, sep=';')

# household_power2machine()